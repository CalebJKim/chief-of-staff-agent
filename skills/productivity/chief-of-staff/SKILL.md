---
name: chief-of-staff
description: Surface deterministically ranked briefs from full Google Workspace context and handle related or direct Workspace actions.
version: 0.9.1
...
- **Daily brief:** Only when the user asks what to work on today or asks to start the day, run `bash "$HERMES_HOME/skills/productivity/chief-of-staff/scripts/start_day.sh"` as the only terminal command. The helper scans bounded Gmail, Calendar, Drive, and Sheet evidence and returns one packet. Python always deterministically ranks Gmail anchors by generic metadata in this fixed precedence: Gmail Important, sole direct recipient, unread, then newest received. The selected entries are already marked `selected_for_output` with a fixed `selection_order`; Calendar, Drive, Sheet, and remaining mail data are supporting context and never change that selection. Produce the final brief without ranking again. The default is three items. If the user specifies a positive item count, append `--top N`; selection remains deterministic regardless of wording. Use the positive integer from the request. Add no preamble, second copy, closing question, or offer.
...
The packet contains bounded Gmail, Calendar, Drive, and Sheet context. Render exactly the `mail` entries whose `selected_for_output` value is true, once each in `selection_order`. Python has already applied the deterministic metadata ranking. Do not score, rank, reorder, merge, replace, or skip selected entries. Use meetings, files, Sheet evidence, and unselected mail only to enrich a selected item when the live packet clearly supports the relationship; supporting context must never become a replacement item or alter the fixed order. Never invent a resource relationship or use remembered, user-learned, or workspace-specific mappings. Preserve the selected message's action state: a request or proposal remains pending, while only an explicit completion statement may be described as completed. When recommending email work, say to prepare or save a draft for review, never to send it. The packet creates recommendations only; it never stores executable actions.

## Initial Reply

Follow the packet's `response_contract` exactly. Output only one workload-summary sentence with no heading or preamble, immediately followed by exactly one numbered item for each `mail` entry marked `selected_for_output`. Item N must be anchored to the selected entry with `selection_order: N`; supporting context may enrich it but may not replace, merge, or reorder it. Render each item as exactly three lines: a bold outcome line; one separate indented sub-bullet labeled `Evidence:` containing a complete sentence and the selected entry's `[Mail — SENDER](URL)` link, plus one matching Calendar, Sheet, Doc, Slides, or Drive link when the packet clearly supports it; and one indented action sub-bullet labeled `Recommended action item(s):` that states only the desired end state and scope. Never invent or link an unrelated resource. Copy sender names exactly from live `mail.from`. Before finalizing, silently verify the item count, selection order, three-line format, exact sender and Mail link for every item, any supporting resource relationship, and that no raw IDs, helper names, flags, commands, row numbers, cell coordinates, backticks, extra sections, closing question, or offer appears. End after the final action line.

1. **Outcome**
   - **Evidence:** What the selected message and matching Workspace context say. [Calendar/Sheet/Doc/Slides/Drive](URL) [Mail — Sender](URL)
...
For a request outside the displayed brief, use the same focused helper directly. Read only the relevant thread, event window, tracker range, document, or deck needed to determine exact values.
