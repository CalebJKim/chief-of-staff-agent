# Public demo seed specification

The seeder creates a self-contained synthetic Chief of Staff environment in the Google account connected through OAuth. It contains no account IDs, credentials, or links from the original demo.
...
- **1 Google Sheet**: `RTX Spark Campaign Tracker — Public Demo`
  - Tab: `Campaign Lanes`
...
- **1 Google Doc**: `RTX Spark Campaign Plan — Public Demo`
- **1 Google Slides deck**: `RTX Spark Exec Review — Public Demo`
  - 10 slides
...
```text
$HERMES_HOME/chief-of-staff-demo-state.json
```

The cleanup command uses this file to delete imported mail/events and move generated Drive files to trash.
...
1. **Sheet** — Create `RTX Spark Campaign Tracker — Public Demo`, tab `Campaign Lanes`, with the A:J columns listed above. Add at least these lanes: Product performance claims (Awaiting update), Exec Review deck (Awaiting update), Agent Messaging (Awaiting update), Marketing shoot (Blocked), Partner enablement (On track), Social rollout (Awaiting update), Retail demo readiness (Blocked), Legal intake (Awaiting update).
2. **Slides** — Create a 10-slide `RTX Spark Exec Review — Public Demo`. Put `Performance to go here - Mike Chen to provide` on slide 4, `Move the detail out of the live flow` on slide 6, and two decision asks on slide 10.
3. **Doc** — Create `RTX Spark Campaign Plan — Public Demo` with an agent-first narrative and open work for claims, retail demo ownership, shoot date, and Exec Review preparation.
